...
const isSuperAdmin = (req: Request, res: Response, next: NextFunction) => {
  if (req.isAuthenticated() && (req.user as IUser).role === 'superadmin') {
    return next();
  }
...
router.get('/superadmin-only', isSuperAdmin, (req: Request, res: Response) => {
...
router.put('/assign-role/:id', isSuperAdmin, async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json({ message: 'Role assigned successfully', user: { id: user._id, username: user.username, role: user.role } });
  } catch (error) {
...
router.get('/users', isSuperAdmin, async (req: Request, res: Response) => {
  try {
...
router.put('/assign-role', isSuperAdmin, assignRole);
